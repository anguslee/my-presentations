...
\begin{frame}{Case Study: Web API搜索服务结构优化（续）}
  \begin{center}
    \begin{tikzpicture}[
        font=\sffamily,
        every matrix/.style={ampersand replacement=\&,column sep=1cm,row sep=.3cm},
      ...
      \matrix{
        ...
        \node[process] (preprocess) {预处理、\\数据挖掘}; \&
        \node[datastore] (processedstorage) {RDBMS}; \&
        \node[datastore] (kvengine) {Key-Value\\存储引擎}; \\
      };
      ...
      \draw[to] (preprocess) to[bend right=40] (processedstorage);
      \draw[to] (processedstorage) to[bend right=40] (preprocess);
      \draw[to] (preprocess) to[bend right=40] (kvengine);
      \draw[to] (kvengine) to[bend left=40] (preprocess);
...
\begin{frame}{优化原则}
  \begin{enumerate}
  \item { 相对外部访问次数，API时间复杂度控制在O(1) }
  \item { 牢记墨菲定律。从底层结构上保证无单点依赖，且能够横向扩展 }
