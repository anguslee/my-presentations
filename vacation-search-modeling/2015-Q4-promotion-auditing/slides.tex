...
\begin{frame}{Case 3: 小流量测试环境构建}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \matrix{
        \&
        \node[sink] (pcfrontend) {度假搜索PC前端};  \&
        \\

        \node[process] (webapi) {Web API\\搜索服务}; \&
        \node[datastore] (accessrecords) {用户行为日志}; \&
        \node<2->[process] (uidhash) {UID \\Hash}; \\

        \&
        \node[sink] (mobilefrontend) {度假搜索Mobile前端}; \&
        \\
      };

      % Draw the arrows between the nodes and label them.
      \draw[to] (webapi) -- (pcfrontend);
      \draw[to] (webapi) -- (mobilefrontend);
      \draw<2->[to] (uidhash) -- (pcfrontend);
      \draw<2->[to] (uidhash) -- (mobilefrontend);
    \end{tikzpicture}
  \end{center}
\end{frame}
